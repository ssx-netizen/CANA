...
def main(args):
	...
	for i in folds:
		...
		if args.task_type=='summary':
			args.results_dir = args.read_results_dir
		results_pkl_path = os.path.join(args.results_dir, 'split_latest_val_{}_results.pkl'.format(i))
		if args.task_type=='survival' and os.path.isfile(results_pkl_path) and (not args.overwrite):
		...
		if args.task_type == 'survival':
			...
			val_latest, cindex_latest, train_latest = eval(datasets, i, args)
			latest_val_cindex.append(cindex_latest)
			save_pkl(os.path.join(args.results_dir, 'split_latest_val_{}_results1.pkl'.format(i)), val_latest)
			save_pkl(os.path.join(args.results_dir, 'split_latest_val_{}_results2.pkl'.format(i)), train_latest)
...
parser.add_argument('--read_results_dir', type=str, default='./results_new', help='Results directory to read from for summary task.')
...
parser.add_argument('--mode',            type=str, choices=['omic', 'path', 'pathomic', 'pathomic_fast', 'cluster', 'coattn', 'omic_sig'], default='coattn', help='Specifies which modalities to use / collate function in dataloader.')
parser.add_argument('--fusion',          type=str, choices=['None', 'concat', 'bilinear', 'mmgf', 'lmf'], default='None', help='Type of fusion. (Default: concat).')
...
parser.add_argument('--bag_loss',        type=str, choices=['svm', 'ce', 'ce_surv', 'nll_surv', 'nll_surv_dep'], default='nll_surv', help='slide-level classification loss function (default: ce)')
parser.add_argument('--label_frac',      type=float, default=1.0, help='fraction of training labels (default: 1.0)')
parser.add_argument('--reg', 			 type=float, default=1e-5, help='L2-regularization weight decay (default: 1e-5)')
parser.add_argument('--alpha_surv',      type=float, default=0.0, help='How much to weigh uncensored patients')
parser.add_argument('--reg_type',        type=str, choices=['None', 'omic', 'pathomic', 'weight'], default='None', help='Which network submodules to apply L1-Regularization (default: None)')
...
parser.add_argument('--task_type', type=str, choices=['survival', 'summary'], default='survival', help='Task type: survival or classification')
...
if 'survival' in args.task:
	...
	#print('./%s/%s_all_clean.csv.zip' % (args.dataset_path, study))
...
if args.task_type=='survival' and ('summary_latest.csv' in os.listdir(args.results_dir)) and (not args.overwrite):
