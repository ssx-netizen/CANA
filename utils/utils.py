...
def collate_MIL_survival(batch):
    ...
    #print([item[2].type for item in batch])
    #label = torch.LongTensor([item[2] for item in batch])
    label = torch.tensor([item[2].tolist() for item in batch], dtype=torch.long).squeeze(1)
    #print(label.shape)
...
def collate_MIL_survival_sig(batch):
    ...
    label = torch.tensor([item[7].tolist() for item in batch], dtype=torch.long).squeeze(1)
...
def l1_reg_w(model, reg_type=None):
    l1_reg = None
    for W in model.parameters():
        if l1_reg is None:
            l1_reg = W.abs().sum()
