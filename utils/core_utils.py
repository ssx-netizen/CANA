...
from models.model_porpoise import PorpoiseMMF, PorpoiseAMIL, LSHAMIL, LSHMMF
from models.model_dsmil import DSMILNet
from models.model_my import Multimodal, SNNSIG, TransMIL
...
def train(datasets: tuple, cur: int, args: Namespace):
    ...
    if args.task_type == 'survival':
        ...
        elif args.bag_loss == 'nll_surv_dep':
            loss_fn = NLLSurvLoss_dep(alpha=args.alpha_surv)
        else:
    ...
    elif args.model_type == 'lsh_amil':
        model_dict = {'n_classes': args.n_classes}
        model = LSHAMIL(**model_dict)
    elif args.model_type == 'lsh_mmf':
        model_dict = {'omic_input_dim': args.omic_input_dim, 'fusion': args.fusion, 'n_classes': args.n_classes, 
        'gate_path': args.gate_path, 'gate_omic': args.gate_omic, 'scale_dim1': args.scale_dim1, 'scale_dim2': args.scale_dim2, 
        'skip': args.skip, 'dropinput': args.dropinput, 'path_input_dim': args.path_input_dim, 'use_mlp': args.use_mlp,
        }
        model = LSHMMF(**model_dict)
    ...
    elif args.model_type == 'snnsig':
        model_dict = {'fusion': args.fusion, 'omic_sizes': args.omic_sizes, 'n_classes': args.n_classes}
        model = SNNSIG(**model_dict)
    elif args.model_type == 'transmil':
        model_dict = {'n_classes': args.n_classes}
        model = TransMIL(n_classes=args.n_classes)
    elif args.model_type == 'dsmil':
        model_dict = {'n_classes': args.n_classes}
        model = DSMILNet(n_classes=args.n_classes)
    else:
        raise NotImplementedError
    ...
    if args.early_stopping:
        early_stopping = EarlyStopping(warmup=10, patience=3, stop_epoch=50, verbose = True)
    else:
        early_stopping = None
    ...
    # model.load_state_dict(torch.load(os.path.join(args.results_dir, "s_{}_checkpoint.pt".format(cur))))
        ...
        #################################################
        if val_cindex > max_cindex:
            torch.save(model.state_dict(), os.path.join(args.results_dir, "s_{}_checkpoint.pt".format(cur)))
            max_cindex = val_cindex
            print('Best val c-Index')

    writer.close()
    return results_val_dict, val_cindex


def eval(datasets: tuple, cur: int, args: Namespace):
    """   
        train for a single fold
    """
    print('\nEvaling Fold {}!'.format(cur))
    ...
    if args.mode == 'coattn':
        results_val_dict2, val_cindex2 = summary_survival_coattn(model, train_loader, args.n_classes)
        results_val_dict, val_cindex = summary_survival_coattn(model, val_loader, args.n_classes)
    else:
        results_val_dict2, val_cindex2 = summary_survival(model, train_loader, args.n_classes)
        results_val_dict, val_cindex = summary_survival(model, val_loader, args.n_classes)

    print('Val c-Index: {:.4f}'.format(val_cindex))
    return results_val_dict, val_cindex, results_val_dict2
    ...
    for batch_idx, (data_WSI, data_omic, y_disc, event_time, censor) in enumerate(loader):
        ...
        if not isinstance(h, tuple):
            ...
            if len(h)==3:
                ...
                h_i, h_b = h 
                loss = 0.5*loss_fn(h=h_b, y=y_disc, t=event_time, c=censor)
                h_m, _ = torch.max(h_i, 0)
                loss += 0.5*loss_fn(h=h_m.unsqueeze(0), y=y_disc, t=event_time, c=censor)
                loss_value = loss.item()
                h = h_b
...
def summary_survival(model, loader, n_classes):
    ...
    for batch_idx, (data_WSI, data_omic, y_disc, event_time, censor) in enumerate(loader):
        ...
        all_event_times[batch_idx] = event_time
        patient_results.update({slide_id: {'slide_id': np.array(slide_id), 'risk': risk, 'hazards': hazards.detach().cpu().numpy(), 'disc_label': y_disc.item(), 'survival': event_time, 'censorship': censor}})
