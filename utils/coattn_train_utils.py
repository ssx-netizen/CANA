...
def train_loop_survival_coattn(epoch, model, loader, optimizer, n_classes, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., gc=16):   
    ...
    for batch_idx, (data_WSI, data_omic1, data_omic2, data_omic3, data_omic4, data_omic5, data_omic6, label, event_time, c) in enumerate(loader):
        ...
        ##############################
        h = model(x_path=data_WSI, x_omic1=data_omic1, x_omic2=data_omic2, x_omic3=data_omic3, x_omic4=data_omic4, x_omic5=data_omic5, x_omic6=data_omic6)
        y_disc = label
        censor = c
        if not isinstance(h, tuple):
            ...
            # loss_value = loss.item()
            # h = h_mm
            logits, ps, pc, ost, oc = h
            loss = loss_fn(h=logits, y=y_disc, t=event_time, c=censor)
            loss += 1*loss_fn(h=ps, y=y_disc, t=event_time, c=censor)
            loss += 1*loss_fn(h=pc, y=y_disc, t=event_time, c=censor)
            loss += 1*loss_fn(h=ost, y=y_disc, t=event_time, c=censor)
            loss += 1*loss_fn(h=oc, y=y_disc, t=event_time, c=censor)
            loss_value = loss.item()
            h = logits
...
def validate_survival_coattn(cur, epoch, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None):
    ...
    for batch_idx, (data_WSI, data_omic1, data_omic2, data_omic3, data_omic4, data_omic5, data_omic6, label, event_time, c) in enumerate(loader):
        ...
        if not isinstance(h, tuple):
            ...
            logits, ps, pc, ost, oc = h
            loss = loss_fn(h=logits, y=y_disc, t=event_time, c=censor)
            loss += Proportion*loss_fn(h=ps, y=y_disc, t=event_time, c=censor)
            loss += Proportion*loss_fn(h=pc, y=y_disc, t=event_time, c=censor)
            loss += Proportion*loss_fn(h=ost, y=y_disc, t=event_time, c=censor)
            loss += Proportion*loss_fn(h=oc, y=y_disc, t=event_time, c=censor)
            loss_value = loss.item()
            h = logits
...
def summary_survival_coattn(model, loader, n_classes):
    ...
    for batch_idx, (data_WSI, data_omic1, data_omic2, data_omic3, data_omic4, data_omic5, data_omic6, label, event_time, c) in enumerate(loader):
        ...
        #########################################################
        with torch.no_grad():
            h = model(x_path=data_WSI, x_omic1=data_omic1, x_omic2=data_omic2, x_omic3=data_omic3, x_omic4=data_omic4, x_omic5=data_omic5, x_omic6=data_omic6) # return hazards, S, Y_hat, A_raw, results_dict
        ...
        all_event_times[batch_idx] = event_time
        patient_results.update({slide_id: {'slide_id': np.array(slide_id), 'risk': risk, 'hazards': hazards.detach().cpu().numpy(), 'disc_label': label.item(), 'survival': event_time, 'censorship': c}})
