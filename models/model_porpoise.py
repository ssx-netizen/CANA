...
class PorpoiseMMF(nn.Module):
    def __init__(self, 
        ...
        # self.size_dict_path = {"small": [path_input_dim, 512, 512], "big": [1024, 512, 384]}
        ...
        if self.Distill:
            self.classifier_omic = nn.Linear(size[2], n_classes)
            self.classifier_path = nn.Linear(size[2], n_classes)
    ...
    def relocate(self):
        ...
        if self.Distill:
            ...
            self.classifier_omic = self.classifier_omic.to(device)
            self.classifier_path = self.classifier_path.to(device)
    ...
    def forward(self, **kwargs):
        ...
        if self.Distill:
            h_omic = self.classifier_omic(h_omic)
            h_path = self.classifier_path(h_path)
...
class MMGF(nn.Module):
    def __init__(self):
        ...
        self.linear_h = nn.Sequential(nn.Linear(512, 512), nn.ReLU())
        # self.linear_h = nn.Sequential(nn.Linear(256, 256), nn.ReLU())
        #init_max_weights(self)

    def forward(self, vec1, vec2):
        ### Gated Multimodal Units
        vec1 = self.linear_h(vec1)
        vec2 = self.linear_h(vec2)
        vec = torch.cat((vec1, vec2), dim=0)
        flatten_vec = vec.view(1, -1)
        a = F.softmax(flatten_vec, dim=1)
        out = torch.mul(flatten_vec, a)
        out = out.view_as(vec)
        out = torch.sum(out, dim=0).unsqueeze(0)
        return out
    
class LSHAMIL(nn.Module):
    def __init__(self, size_arg = "small", n_classes=4):
        super(LSHAMIL, self).__init__()
        self.bucket_size = 128
        self.attn = LSHSelfAttention(dim=1024, heads=4, bucket_size=self.bucket_size, n_hashes=2, causal=False)
        self.size_dict = {"small": [1024, 512, 256], "big": [1024, 512, 384]}
        size = self.size_dict[size_arg]
    ...
    def forward(self, **kwargs):
        ...
        h = h.unsqueeze(0)
        b, t, d = h.shape
        paddings = torch.zeros(b, 2*self.bucket_size - (t % (2*self.bucket_size)), d, device=h.device)
        # print(h.shape)
        h = torch.cat((h, paddings), dim=1)
        # print(h.shape)
        input_mask = torch.ones(b, t, device=h.device).bool()
        paddings_mask = torch.zeros(b, 2*self.bucket_size - (t % (2*self.bucket_size)), device=h.device).bool()
        input_mask = torch.cat((input_mask, paddings_mask), dim=1)
        h = self.attn(h, input_mask=input_mask)
        h = h.squeeze(0)
        h = h[:t]
        A, h = self.attention_net(h)  
        A = torch.transpose(A, 1, 0)
